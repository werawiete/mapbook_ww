...
def update_users(idx):
    ...
    users[idx] = map_widget.set_marker(users[idx].coordinates[0],users[idx].coordinates[1])
...
button_pokaz_szczegoly=Button(ramka_lista_obiektow,text="Pokaż szczegóły", command=user_detales)
...
button_dodaj_obiekt=Button(ramka_formularza, text="Dodaj", command=add_user)
...
map_widget.set_position(deg_x=52.23, deg_y=21)
